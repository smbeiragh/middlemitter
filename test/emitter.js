...
describe('MiddlEmitter advance features', function() {
  describe('#on', function() {
    it('should add listeners with different priority', function() {
      ...
      return emitter.emit('test', () => { }).then(() => {
        expect(calls, 'expect to listeners execute from height to low priority an in order').to.deep.equal([1,2,3,4,5,6]);
      });
    });
    ...
    it('should add same listener to multiple events', function() {
      const emitter = new MiddlEmitter();
      const calls = [];
      const listener = (log) => { calls.push(log) }
  ...
  describe('#once', function() {
    it('should add listeners with different priority', function() {
      const emitter = new MiddlEmitter();
      const calls = [];
      emitter.once('test', () => { calls.push(6) });
      emitter.once('test', () => { calls.push(5) }, 10);
      emitter.once('test', () => { calls.push(2) }, 20);
      emitter.once('test', () => { calls.push(3) }, 20);
      emitter.once('test', () => { calls.push(4) }, 15);
      emitter.once('test', () => { calls.push(1) }, 30);

      return emitter.emit('test', () => { }).then(() => {
        expect(calls, 'expect to call listeners from height to low priority an in order').to.deep.equal([1,2,3,4,5,6]);
      });
    });
  ...
  describe('#off', function() {
    ...
    it('should remove listener when same listener attached to multiple events via once method', function() {
      ...
      return emitter.emit('test1', 'test1 log')
        ...
        .then(() => emitter.emit('test3', 'test3 log'))
        .then(() => {
          expect(calls, 'expect to remove listener from an event while it\'s attached via once and it\'s attached to other events')
  ...
  describe('async listeners', function() {
    it('should await on async listeners', function() {
      ...
      return emitter.emit('test', () => { }).then(() => {
        expect((finish - start) >= 400, 'expect to await on async listeners and execute next listener just after async listener exec finished')
          .to.equal(true);
      });
    });
  });
